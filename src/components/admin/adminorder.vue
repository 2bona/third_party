...
<p class=" font-weight-black body-1 text-right"><v-icon style="padding-bottom:2px" color="black" size="15">mdi-currency-ngn</v-icon>{{order.grand_total | price}}.00</p>
 ...
 <v-divider></v-divider>
  <div>
      <v-layout v-if="!(order.status === 4) && !(order.status === 3)"  style="position:fixed; bottom:0px;background: rgb(245, 245, 245);width: 100%; z-index:9" row wrap class=" pb-2 px-2">
        <v-flex xs6 class="px-2">
        <v-btn  :loading="loading" block @click="reject()" class="mt-2 elevation-10" rounded="" dark color="orange darken-4">reject</v-btn>
        </v-flex>
        <v-flex xs6 class="px-2">
        <v-btn :loading="loading" block @click="serve()" class="mt-2 elevation-10" rounded="" dark color="primary">served</v-btn>
        </v-flex>
      </v-layout>
  </div>
</v-card>
