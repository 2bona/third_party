    ...
    <v-expansion-panel>
      ...
      <v-expansion-panel-content>
          <v-row justify="space-around">
           <v-btn @click="dialog = true" text x-small color="grey"><v-icon>mdi-plus</v-icon>add item</v-btn>
           </v-row> 
         ...
         <v-slide-x-reverse-transition>
          <v-flex v-show="n.status" xs5 style="
            position: absolute; right: -2px; z-index:99; top: -3px;">  
              <v-btn  @click="editMainOption(n.id, n.name, n.title, n.option, n.max)"  icon text color="grey lighten-2" small class="mt-1 mr-1">
                   <v-icon>mdi-pencil-outline</v-icon>
                    </v-btn>
                      <v-btn icon text @click="deleteMainOption(n.id, n.name)" color="grey lighten-2" small class="mt-1 mr-1">
                        <v-icon>mdi-trash-can</v-icon>
                    </v-btn>
                  </v-flex>
            </v-slide-x-reverse-transition>
                </v-layout>
                <p style="padding-left:10px;" :class="!n.status? 'text-truncate' : 'text-wrap'" class=" text-left  caption font-weight-regular grey--text mt-1 mb-0">{{n.title}}</p>
                        <v-flex xs12>
                <v-expand-transition>
          <v-layout v-show="n.status" style="width:100%;padding-left: 13px !important;" row wrap class="py-1"> 
      <v-flex xs12> 
        <p class="overline my-0 py-0 grey--text font-weight-bold text-capitalize">options</p>
            <v-divider class=" grey lighten-4 mb-1"></v-divider>
        <v-layout row wrap class="pl-3">
            <v-flex style="max-width:40px;display: inline-grid;" class="mb-2 mr-2" v-for="d in n.option" :key="d.image">
          <v-avatar
            size="25px" class="mx-auto elevation-2"
            color="transparent"
          ><img :src="d.image" :alt="d.name">
          </v-avatar> 
          <p class="overline grey--text pl-1 mb-0 text-capitalize text-center text-truncate">{{d.name}}</p>
          </v-flex>   </v-layout>      
      </v-flex>
          </v-layout> 
                </v-expand-transition>    
